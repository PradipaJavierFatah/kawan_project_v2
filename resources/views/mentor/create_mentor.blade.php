...
<body>
    <div class="container mt-5">
        <h2 class="text-center mb-4">Create New Mentor</h2>
        <!-- Form untuk Create atau Edit Mentor -->
        <form action="{{ isset($mentor) ? route('mentors.update', $mentor->id) : route('mentors.store') }}" method="POST"
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                    name="name" value="{{ old('name', $mentor->name ?? '') }}">
                @error('name')
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('age') is-invalid @enderror" id="age"
                    name="age" value="{{ old('age', $mentor->age ?? '') }}">
                @error('age')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                    rows="3">{{ old('description', $mentor->description ?? '') }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                @if (isset($mentor) && $mentor->picture)
                    <img src="{{ asset($mentor->picture) }}" alt="Current Picture"
